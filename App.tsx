...
function App() {
  ...
  useEffect(() => {
    ...
    const initData = async () => {
      // 首先检查是否需要认证（记录是否设置了密码，用于后续判断是否能修改）
      try {
  ...
  const SortableLinkCard = ({ link }: { link: LinkItem }) => {
    ...
    return (
      <div
        ref={setNodeRef}
        style={style}
        className={`group relative transition-all duration-200 cursor-grab active:cursor-grabbing min-w-0 max-w-full overflow-hidden hover:shadow-lg hover:shadow-blue-100/50 dark:hover:shadow-blue-900/20 ${isSortingMode || isSortingPinned
              ...
              <div className="flex items-center gap-2">
                ...
                <div className={`${isMobileSearchOpen ? 'hidden' : 'flex'} lg:flex items-center bg-slate-100 dark:bg-slate-700 rounded-full p-1`}>
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-medium rounded-full transition-all ${viewMode === 'compact'
                      ? 'bg-white dark:bg-slate-600 text-blue-600 dark:text-blue-400 shadow-sm'
                      : 'text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-slate-100'
                      }`}
                    title="简约版视图"
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-medium rounded-full transition-all ${viewMode === 'detailed'
                      ? 'bg-white dark:bg-slate-600 text-blue-600 dark:text-blue-400 shadow-sm'
                      : 'text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-slate-100'
                      }`}
                    title="详情版视图"
